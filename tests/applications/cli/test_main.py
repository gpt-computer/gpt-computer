...
import gpt_computer.applications.cli.main as main

from gpt_computer.applications.cli.main import load_prompt
...
class TestMain:
    #  Runs gpt-computer cli interface for many parameter configurations, BUT DOES NOT CODEGEN! Only testing cli.
    ...
    #  Runs gpt-computer with improve mode and improves an existing project in the specified path.
    ...
    #  Runs gpt-computer with improve mode and improves an existing project in the specified path, with skip_file_selection
    ...
    #  Runs gpt-computer with clarify mode and generates a project after discussing the specification with the AI.
    ...
    #  Runs gpt-computer with self-heal mode and generates a project after discussing the specification with the AI and self-healing the code.
...
class TestLoadPrompt:
    ...
    async def test_load_prompt_no_file_improve_mode_false(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert isinstance(result, Prompt)
            assert (
                result.text == "What application do you want gpt-computer to generate?"
            )
            assert result.image_urls is None
...
#         monkeypatch.setattr("builtins.input", lambda _: next(gen))
#         monkeypatch.setattr(
#             "gpt_computer.core.default.steps.salvage_correct_hunks",
